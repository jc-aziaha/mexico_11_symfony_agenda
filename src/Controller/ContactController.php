...
class ContactController extends AbstractController
{
    ...
    public function create(Request $request, EntityManagerInterface $em): Response
    {
        ...
        if ( $form->isSubmitted() && $form->isValid() ) 
        {
            ...
            // $this->addFlash("success", $contact->getFirstName() . " " . $contact->getLastName() . " a été ajouté à la liste.");
    ...
    public function delete(Contact $contact, Request $request, EntityManagerInterface $em): Response
    {
        if ( $this->isCsrfTokenValid('delete_contact_'.$contact->getId(), $request->request->get('csrf_token')) ) 
        {
            ...
            $this->addFlash("success", "Le contact <em>{$contact->getFirstName()} {$contact->getLastName()}</em> a été retiré de la liste.");
